...
#### 原理总结:
 例子中，有两方法，一个有@Transational注解，一个没有。     
...
#####  4.5.3.1、jpaTransactionManager 和 DataSourceTransactionManage
...
@Configuration
//jpa 扫描配置
//EnableJpaRepositories 中 entityManagerFactoryRef 有默认值entityManagerFactory （写了EntityScan，）不写也可以（不写它，如果要使用jpa这样就必须写，EntityScan了）
//EnableJpaRepositories 中 transactionManagerRef   有默认值 transactionManager
