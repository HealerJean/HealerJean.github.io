...
### 2.16.6、` to_days(date)`, `from_days(days)`  ：
...
### 2.16.7、`time_to_sec(time)`,` sec_to_time(seconds)`  

> `time_to_sec(time)`,` sec_to_time(seconds)`  （时间、秒（互转）） 
...
### 2.16.8、 `makdedate(year,dayofyear)`, `maketime(hour,minute,second)  `
