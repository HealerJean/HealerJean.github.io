...
```
CREATE TABLE demo_tmp LIKE demo
rename table 
    demo to demo_bak,
    demo_tmp to demo;
```
...
> - ON 过滤：只过滤关联数据，**不影响主表数据**:：**先会在副表中对and条件进行过滤，然后再跟左边主表进行关联**

1. **关联时就过滤**：
2. 不满足这个条件的行，**直接不参与 JOIN**
3. 最后统计数量
...
1. **先做 JOIN 关联**：把两张表按 `route_no` 全部匹配，生成**大结果集**
2. **后过滤**：从关联后的结果里，只保留 `v.status = '4'` 的数据
3. 最后统计数量
