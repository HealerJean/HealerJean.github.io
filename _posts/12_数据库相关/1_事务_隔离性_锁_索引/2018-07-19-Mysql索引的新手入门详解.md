...
- MySQL 的查询优化器依赖索引的**基数（cardinality）** 和**选择性（selectivity）** 来决定是否使用索引。
- 如果一列大量为 `NULL`，会导致：
  - 估算行数不准（比如 `WHERE col = 'X'` 时，优化器可能高估或低估匹配行数）。
  - 可能导致本该走索引的查询走了全表扫描。
...
![image-20220527162625808](/Users/healerjean/Desktop/HealerJean/HCode/HealerJean.github.io/blogImages/image-20220527162625808.png)
...
> ##### 解释 ：可以通过 `key`_`len` 的长度来判断联合索引使用到了那些     
...
**2.1、使用`is null` /`is not null`会进行全表扫描，索引无法存储`null`值，所以where的判断条件如果对字段进行了`null`值判断，将导致数据库放弃索引而进行全表查询**
...
**2.2、使用`where IN()`过滤时，IN集合个数必须小于500，因为in的数据少的时候，mysql优化器会可能会使用索引（在in里面的列表值mysql是会自动从小到大排序，加锁也是一条条从小到大加的锁）**，**但是当数据太多以后就不一定了，可以让MySQL按照ID顺序进行查询**，**这可能比随机的关联要更高效，如**        
