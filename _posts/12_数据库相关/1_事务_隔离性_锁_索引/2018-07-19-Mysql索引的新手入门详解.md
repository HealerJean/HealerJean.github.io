...
**2.2、使用where IN()过滤时，IN集合个数必须小于500，因为in的数据少的时候，mysql优化器会可能会使用索引（在in里面的列表值mysql是会自动从小到大排序，加锁也是一条条从小到大加的锁）**，**但是当数据太多以后就不一定了，可以让MySQL按照ID顺序进行查询**，**这可能比随机的关联要更高效，如**        
