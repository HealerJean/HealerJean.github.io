...
![image-20201109160748697](D:\study\HealerJean.github.io\blogImages\image-20201109160748697.png)
...
![image-20201109161047848](D:\study\HealerJean.github.io\blogImages\image-20201109161047848.png)



![image-20201109161250293](D:\study\HealerJean.github.io\blogImages\image-20201109161250293.png)
...
1、先通过`SPI`方式获取`ProxyFactory `创建`Invoker`，    

2、通过`SPI`的方式获取`Protocol`实现导出。    
...
![image-20201109175208925](D:\study\HealerJean.github.io\blogImages\image-20201109175208925.png)



![image-20201109175555258](D:\study\HealerJean.github.io\blogImages\image-20201109175555258.png)
...
> `JavassistProxyFactory` 的方法：`#getInvoker`创建了一个继承自` AbstractProxyInvoker` 类的匿名对象，并覆写了抽象方法 `#doInvoke`。覆写后的 `#doInvoke` 逻辑比较简单，仅是将调用请求转发给了 `Wrapper` 类的` invokeMethod `方法。   （` invoke`方法后面讲）   
...
>
> 在创建 `Wrapper` 子类的过程中，子类代码生成逻辑会对 `getWrappe`r 方法传入的 `Class` 对象进行解析，拿到类方法，类成员变量等信息。以及生成 `invokeMethod` 方法代码和其他一些方法代码。      再通过反射获取`wpper`对象   
...
> 在创建 `Wrapper` 子类的过程中，子类代码生成逻辑会对 `makeWrapper`方法传入的 Class 对象进行解析，拿到诸如类方法，类成员变量等信息。以及生成 `invokeMethod` 方法代码和其他一些方法代码。代码生成完毕后，再通过反射创建 Wrapper 实例。    
...
#### 3.1.1.4、最终返回的`AbstractProxyInvoker`

> 有一个未实现的抽象方法`doInvoke`
...
### 3.1..2、本地协议`InjvmProtocol` 导出

> 在获取Invoker之后，就会调用具体的`Protocol`实现类导出方法进行导出，首先是导出到本地，使用的是`InjvmProtocol`协议，导出逻辑很简单，源码如下：
...
> 接着上面的源码分析导出服务方法`doLocalExport`的源码。
...
![image-20201110180237572](C:\Users\HealerJean\AppData\Roaming\Typora\typora-user-images\image-20201110180237572.png)
...
第一是检测是否存在`url`中 `server` 参数所代表的 `Transporter `拓展(默认是`netty`)，不存在则抛出异常。      

第二是创建服务器实例。     

第三是检测是否支持`url`中`client` 参数所表示的` Transporter` 拓展，不存在也是抛出异常。     
...
> 接下来就主要看看Exchangers是如何创建出`ExchangeServer`服务的
...
> 默认的`Transporter` 是`NettyTransporter`,不过是注意是netty4.x，dubbo也提供了3.x的实现。
>
...
![image-20201110194524012](C:\Users\HealerJean\AppData\Roaming\Typora\typora-user-images\image-20201110194524012.png)
