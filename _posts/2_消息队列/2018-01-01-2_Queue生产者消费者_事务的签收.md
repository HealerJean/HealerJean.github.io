...
> **`QUEUE` 默认是持久化的，也就是说，当`activemq`挂掉再重启，还是可以看到的** 
...
`ActiveMQ`：默认的持久化方式， 如果关闭`ActiveMQ` 再开启，造成消息丢失，浏览器中会存在关于该`queue`这个东西，但是全部数值为0  

`Mysql`：持久化方式，如果关闭`ActiveMQ`，再开启，造成消息丢失，浏览器中什么都看不到，不存在该`queue`这个东西
...
> `message.acknowledge()`;    
>
> 使用时，如果客户端已经消费了消息，则使用该方法，对`message`进行签收，则会签收所有被使用的消息，再次调用消费者，则不会收到数据              
>
> 
>
