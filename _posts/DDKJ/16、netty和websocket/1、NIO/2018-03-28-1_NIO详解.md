...
　　Buffer是一个对象，它包含一些要写入或读出的数据。在NIO中，数据是放入buffer对象的，而在IO中，数据是直接写入或者读到Stream对象的。应用程序不能直接对 Channel 进行读写操作，而必须通过 Buffer 来进行，<font color="red" size="5">  **即 Channel 是通过 Buffer 来读写数据的。**</font>     
...
　　<font color="red" size="5">flip、clear这两个方法便是用来设置这些值的。</font>    
　　
　　
　　1、flip() 源码
