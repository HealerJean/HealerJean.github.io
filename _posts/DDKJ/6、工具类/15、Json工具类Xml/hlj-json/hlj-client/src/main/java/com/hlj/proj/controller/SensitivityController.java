...
public class SensitivityController {
    ...
    @ApiOperation(value = "Json工具脱敏：建议使用",
            ...
            response = SensitivityDTO.class)
    @GetMapping(value = "sensitivity/jsonUtils", produces = "application/json; charset=utf-8")
    @ResponseBody
    public String sensitivity(SensitivityDTO sensitivityDTO) {
        log.info("脱敏--------Json工具脱敏------数据信息{}", sensitivityDTO);
        sensitivityService.getDemoDTO(sensitivityDTO);
        return ResponseBean.buildSensitivitySuccess(sensitivityDTO);
    }
    ...
    @ResponseBody
    public ResponseBean anno(SensitivityDTO sensitivityDTO) {
        log.info("脱敏--------注解脱敏------数据信息{}", sensitivityDTO);
        sensitivityService.getDemoDTO(sensitivityDTO);
        return ResponseBean.buildSuccess(sensitivityDTO);
    }
    ...
    @ResponseBody
    public ResponseBean log(SensitivityDTO sensitivityDTO) {
        log.info("脱敏--------日志脱敏------脱敏数据{}", sensitivityDTO);
        return ResponseBean.buildSuccess(sensitivityDTO);
    }
