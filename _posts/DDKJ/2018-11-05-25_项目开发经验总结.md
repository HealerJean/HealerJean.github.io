...
<font color="red"> 大量数据中如果包含个别不是正常数据，或者有某一特定类型的数据存在错误，要使用独立事物，防止一块臭肉坏了一锅新汤，而且进来捕获单个事物的异常，然后继续执行外面未执行的任务
...
返回结果的data为`ResRecordModel`

#### 6.3、返回集合

请求参数的data为`ReqRecordListData`<br/>

返回结果的data为`ResRecordListModel`
