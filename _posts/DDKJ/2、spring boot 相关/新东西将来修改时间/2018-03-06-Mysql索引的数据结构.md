...
注意：对二叉查找树进行中序遍历，得到有序集合。 先序遍历可以查找数据
...
### 4.2、平衡二叉树（Self-balancing binary search tree）  

左右两个子树的高度差(平衡因子)的绝对值不超过1，平衡二叉树必定是二叉搜索树，平衡二叉树的常用实现方法有AVL、红黑树、替罪羊树、Treap、伸展树等
