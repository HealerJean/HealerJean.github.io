...
**实现：**用了`refreshAfterWrites` 之后，需要自己实现 `CacheLoader` 的 `reload` 方法，在方法中创建一个 `ListenableFutureTask`，然后将这个`task`提交给线程池去异步执行，`reload `方法最后返回这个 `ListenableFutureTask`。这样做之后，缓存失效后重新加载就变成了异步。**加载期间尝试获取缓存的线程也都不会被`block`，而是获取到加载之前的值。当加载完毕之后，各个线程就能取到最新值了**。      
