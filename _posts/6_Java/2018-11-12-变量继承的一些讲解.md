...
- **子类继承自父类的变量，其实是相当于复制了一个副本，不会改变父类自身变量的值（不同于static变量，static是全局的，static会改变）**
- **如果变量和父类如果不重复，== 赋值和 set变量赋值结果是一样的**
...
+  **变量直接赋值（son.name   father.name），改变的自己的变量值或者是父类的变量值，如果setName，肯定是看被实例化的对象是谁，那就给自己赋值啊，因为调用的方法肯定是自己内部的。
  **
