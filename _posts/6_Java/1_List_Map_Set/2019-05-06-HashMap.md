...
（2）如果两个对象相同， `equals`方法一定返回 `true`，这两个对象的`HashCode`一定相同；**除非重写了`HashCode`方法，及其特殊情况下，才回这样重写**    
...
> `Java` 中的集合（`Collection`）有两类，一类是`Lis`t，再有一类是`Set`。前者集合内的元素是有序的，元素可以重复；后者元素无序，但元素不可重复。     
>
...
#### a、`putTreeVal`，红黑树放入数据

1、如果该元素键的 `hash` 值小于当前节点的 `hash` 值的时候，就会作为当前节点的左节点，hash值大于当前节点hash值得时候作为当前节点的右节点。   
...
#### b、`comparableClassFor` 判断是否实现了 `Comparable`
...
#### d、`tieBreakOrder` 使用定义的一套规则来比较 k 和 p 节点的 key 的大小，用来决定向左还是向右查找
...
> 提高 `HashMap` 性能，解决了由于哈希碰撞链表过长导致索引效率慢的问题。利用红黑树增删改查的特点，时间复杂度从`O(n)`降低为`O(logn)`





### 4）`HashMap` 为什么右移 16 和使用 & 与运算代替模运算
...
### 6）我们自定义 `HashMap` 容量最好是多少

-  **负载因子（Load Factor）默认是 0.75**

  - 当 `size > capacity * loadFactor` 时，`HashMap` 会触发扩容。

  - 默认 `loadFactor = 0.75`，所以当元素数量超过容量的 75% 时，就会扩容。

- **容量必须是 2 的幂**：`HashMap` 内部会将你传入的初始容量**向上取整为最近的 2 的幂**。

  - 比如你写 `new HashMap<>(3)`，实际容量会被调整为 `4`；

  - `new HashMap<>(5)` → 实际容量是 `8`；

  - `new HashMap<>(12)` → 实际容量是 `16`。
...
- **如何计算“合适的初始容量”？**
  ...
  - **错误 1：直接传 `N`**
    ...
    ```java
    new HashMap<>(10); // 想存 10 个
    ```
  
    - 内部容量会被设为 **16**
    - 但 16 × 0.75 = **12** → 其实够用 （这个例子碰巧可以）
    - 但如果 `N=13`，`new HashMap<>(13)` → 容量=16 → 最多存 12 个 → **第 13 个触发扩容！** 
  
    **错误 2：忘记除以 `load factor`**
  
    ```java
    new HashMap<>(16); // 以为能存 16 个
    ```
...
2、扩容的阀值 threshold，当键值对的数量超过这个阀值会产生扩容， **Hash数组 每次会扩容长度为以前的2倍**      

3、 **不要轻易改变负载因子，负载因子过高（不会轻易扩容）会导致链表过长，查找键值对时间复杂度就会增高，负载因子过低（一下子就会扩容了）会导致 `hash` 桶的 数量过多，空间复杂度会增高**      
...
### 8）`Jdk1.8` 和 `Jdk1.7`中 `hashmap`区别 
...
1. 哈希函数：对于键 `k`，先计算其哈希值 `h(k)`，然后通过取模得到初始索引：
...
#### c、为什么 `Int2ObjectOpenHashMap` 使用线性探测？

> `FastUtil` 选择线性探测主要出于以下考虑：

- 注意：线性探测容易导致**聚集（`clustering`）** —— 连续的占用块会变长，降低性能。但在 `FastUtil` 的精心实现（如合理扩容策略、高质量哈希函数）下，这个问题被有效缓解。

1. **实现简单**：探测逻辑只需 `i++`。
2. **缓存友好**：访问内存是连续的，`CPU` 缓存命中率高。
3. **性能优势**：在负载因子较低（如 ≤ 0.75）时，平均查找/插入时间接近 `O(1)`，且常数因子小。
4. **专为原始类型优化**：`Int2ObjectOpenHashMap` 专用于 `int → Object` 映射，避免了装箱/拆箱，配合开放寻址进一步提升效率。



### 2）原始类型优化（`Primitive Optimization`）
...
- 默认负载因子（`load factor`）
  ...
  - `FastUtil` 的默认 `load factor` 是 **`0.75f`**（和 Java HashMap 一样）
    - 扩容方式和 `hashmap` 一致
  
- **不要传比实际需要更小的值**，否则会提前扩容（影响性能）
...
public class Int2ObjectMapExample {
