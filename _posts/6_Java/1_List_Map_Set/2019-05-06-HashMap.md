...
> `Map.keySet()` 本身非常快 —— 是 O(1) 时间复杂度，几乎无开销。

## 1）误解

| 误解                               | 事实                                       |
| ---------------------------------- | ------------------------------------------ |
| “`keySet()` 会把所有 key 复制出来” | 错！它只是一个动态视图，数据仍在原 map 中  |
| “百万数据调用 keySet() 很慢”       | 错！无论 map 多大，`keySet()` 都是常数时间 |
| “keySet() 占很多内存”              | 错！只增加一个轻量级包装对象（几十字节）   |

### 2）**原理**

- `keySet`、`values`  两个字段 **不是存储数据的容器**，而是 **缓存的“视图对象”（view objects）**。
- 作用是：**避免重复创建相同的 Set / Collection 视图**，提升性能。
- 而真正的键值对数据，始终存在 `HashMap` 的 **`Node<K,V>[] table` 数组（或红黑树）里**。
...
### 7）`Jdk1.8` 和 `Jdk1.7`中 `hashmap`区别 
