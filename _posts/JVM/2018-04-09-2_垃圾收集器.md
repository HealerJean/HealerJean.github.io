...
<br/>
我们来看日志的[GC (System.gc())，内存变化为：5980K->2760K(251392K)，5980K代表回收前的内存大小，2760K代表回收后的内存大小，251392K代表内存总大小。因此可以得知内存回收大小为（5980K-2760K）K。这就说明JDK8的HotSpot虚拟机并没有采用引用计数算法来标记内存，它对上述代码中的两个死亡对象的引用进行了回收。(因为内存变小，肯定是回收了，要不然能变么)
