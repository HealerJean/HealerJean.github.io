...
> 分四层配置（优先级从高到低：CLI 参数 > 环境变量 > settings.json 语言配置 > 全局强制提示词），同时解决**CLI 界面菜单、报错、帮助** + **AI 回答 / 代码注释 / 文档**全部简体中文，杜绝中英文混杂。

```
┌─[] - [~/.claude] - [3777]
└─[$] cat CLAUDE.md                                                  [14:32:43]
# 全局偏好

- 始终用简体中文回复,除非用户明确要求换语言。
- 代码内标识符、日志、注释保持英文,不要机翻。
```
