...
- **存储范围**：`'1970-01-01 00:00:01' UTC` 到 `'2038-01-19 03:14:07' UTC`
  ...
  - 不建议使用了
- **最大年份**：**2038 年**
- **原因**：`TIMESTAMP` 使用 32 位有符号整数存储 Unix 时间戳（秒数），存在著名的 **“2038 年问题”**。

- 场景：`timestamp`类型是带有时区信息的。一旦你系统中的时区发生改变，例如你修改了时区，该字段的值会自动变更。这个特性用来做一些国际化大项目，跨时区的应用时，特别注意！    
...
- **最大年份**：**9999 年**
- `DATETIME` 不依赖 `Unix` 时间戳，而是直接存储日期和时间，因此**没有 2038 限制**。
- 支持 `DEFAULT CURRENT_TIMESTAMP` 和 `ON UPDATE CURRENT_TIMESTAMP`（从 MySQL 5.6.5 开始）。
