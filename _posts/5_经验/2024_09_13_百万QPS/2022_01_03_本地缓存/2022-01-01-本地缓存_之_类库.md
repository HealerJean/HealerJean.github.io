...
> `Guava` `Cache` 的设计哲学认为：**缓存的主要目的是为了加速读取，如果缓存了 `null`，就无法区分“缓存未命中”和“缓存了空值”这两种状态。**

- **使用 `get(key)` 时：** 如果 `load()` 返回 `null`，会抛出 **`ExecutionException`**（通常包装着 `InvalidCacheLoadException`）。
- **使用 `getUnchecked(key)` 时：** 会抛出 **`UncheckedExecutionException`**。

- 最佳解决方案：使用 `Optional`

  > 如果需要缓存“查不到数据”这种情况（例如为了防止**缓存穿透**，即恶意攻击频繁查询不存在的 ID），官方推荐的做法是使用 `Java` 8 的 `Optional` 类来包装结果。

  ```java
  // 1. 定义缓存类型为 Optional<User>
  LoadingCache<Long, Optional<User>> userCache = CacheBuilder.newBuilder()
      .expireAfterWrite(10, TimeUnit.MINUTES) // 设置过期时间
      .maximumSize(1000)
      .build(new CacheLoader<Long, Optional<User>>() {
          @Override
          public Optional<User> load(Long key) {
              // 2. 使用 Optional.fromNullable 包装可能为 null 的结果
  ...
  // 3. 获取数据
  Optional<User> userOpt = userCache.getUnchecked(100L);
...
1. **绝对不要让 `load()` 方法直接返回 `null`**。
2. 如果业务允许数据不存在，请使用 **`Optional`** 包装返回值。
3. 如果你只是想简单查一下缓存，不想触发自动加载逻辑，可以使用 **`getIfPresent(key)`**，它返回 `null` 是安全的。

| 方法                | 缓存未命中         | `load()` 返回 `null` | 特点                                         |
| ------------------- | ------------------ | -------------------- | -------------------------------------------- |
| `get(key)`          | 触发 `load()` 方法 | 抛出异常             | 自动加载，线程安全，但必须处理 null 情况。   |
| `getUnchecked(key)` | 触发 `load()` 方法 | 抛出运行时异常       | 不需要 `try-catch`，但一旦出错程序可能崩溃。 |
| `getIfPresent(key)` | 直接返回 `null`    | `N/A`                | **不会报错**。只查缓存，不自动加载           |
