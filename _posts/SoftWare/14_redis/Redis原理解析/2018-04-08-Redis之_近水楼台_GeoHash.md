...
在 `Redis` 里面，经纬度使用 `52` 位的二进制整数（使用`zset`获取到的是十进制数哦）进行编码，放进了 `zset` 里面，**`zset` 的 `value` 是元素的 `key`，`score` 是 `GeoHash` 的 `52` 位二进制整数(十进制哦)值。`zset` 的 `score` 虽然是浮点数， 但是对于 `52` 位的整数值，它可以无损存储**。    
