...
public class UserController extends BaseController {
    ...
    public ResponseBean addUser(@RequestBody UserDTO userDTO) {
        ...
        return ResponseBean.buildSuccess("用户添加成功");
    }
    ...
    public ResponseBean login(@RequestBody UserDTO userDTO, HttpServletRequest request, HttpServletResponse response) {
        ...
        verifyCodeDTO.setRandomId(token);
        ...
        Session tokenSession = subject.getSession();
        //4、登陆成功清空登录次数
        securityService.loginTimeLimitCount(result.getUserName(), true);
        response.setHeader(AuthConstants.HEADER_TOKEN_NAME, tokenSession.getId().toString());
