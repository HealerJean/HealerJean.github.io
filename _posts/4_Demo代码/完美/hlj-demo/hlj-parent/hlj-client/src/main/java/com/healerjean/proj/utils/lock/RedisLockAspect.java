...
public class RedisLockAspect {
     ...
     */
    @Around(value = "(execution(* *(..)) && @annotation(redisLock))", argNames = "joinPoint,redisLock")
    public Object doAroundAdvice(ProceedingJoinPoint joinPoint, RedisLock redisLock) throws Throwable {
        if (null == redisLock.value()) {
        ...
        String redisKey = redisLock.value().join(lockKey);
        boolean lockResult = redisService.lock(redisKey, threadId, redisLock.value().getExpireSec());
        if (!lockResult) {
