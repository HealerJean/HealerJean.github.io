...
##### 5.5.3.3、parallel Scavenge ((并行收集器，吞吐量收集器)  
...
##### 5.5.3.4、Serial Old收集器 （串行收集器）



###### 5.5.3.4.1、收集对象：老年代  

###### 5.5.3.4.2、采用算法：标记-整理
...
##### 5.5.3.5、`CMS`收集器（Concurrent Mark Sweep）  

###### 5.5.3.5.1、收集对象：老年代  

###### 5.5.3.5.2、采用算法：标记-清除
...
### 6.1、`jps` ：(jvm  process status 虚拟机进程状态)
...
#### 6.1.1、jps -l ：进程号、输出主类或者jar的完全路径名、  
...
#### 6.1.2、jps –v：输出jvm参数(查看已经运行的main的JVM参数)
...
#### 6.1.3、jps -q ：值显示java进程号，什么都看不出来 
...
### 6.2、`jstat` ：(JVM Statistics Monitoring Tool虚拟机统计信息监控工具)
...
#### 6.2.1、`jstat -class pid` ：查看类加载情况的统计
...
#### 6.2.2、`jstat -gc  pid time num`：实时监控GC容量  
...
![1578378375256](D:\study\HealerJean.github.io\blogImages\1578378375256.png)
...
#### 6.2.3、`jstat -gcutil  pid time num`：实时监控GC百分比
...
![1578378744168](D:\study\HealerJean.github.io\blogImages\1578378744168.png)
...
#### 6.2.4、`jstat -gcnewcapacity pid`: 年轻代内存统计    
...
![1578381601165](D:\study\HealerJean.github.io\blogImages\1578381601165.png)  
...
#### 6.2.5 、`jstat -gcnew pid`：年轻代垃圾回收统计
...
#### 6.2.6、`jstat -gcoldcapacity  pid`：老年代内存统计  
...
#### 6.2.7、`jstat -gcold  pid `：老年代垃圾回收统计  
...
#### 6.2.8、`jstat -gcmetacapacity   pid`：元数据内存统计 
...
| 参数         | 说明                                   |
| ------------ | -------------------------------------- |
| MCMN、MCMX   | 最小元数据容量、最大元数据容量         |
| MC           | 当前元数据空间大小、                   |
| CCSMN、CCSMX | 最小压缩类空间大小、最大压缩类空间大小 |
| CCSC         | 当前压缩类空间大小                     |
| YGC          | 年轻代gc次数                           |
| FGC          | 老年代gc次数                           |
| FGCT         | 老年代gc时消耗时间                     |
...
### 6.3、`jstack  pid`：显示虚拟机的线程快照，定位死循环、线程阻塞、死锁等问题 
...
![1578384566064](D:\study\HealerJean.github.io\blogImages\1578384566064.png)
...
![1578384656158](D:\study\HealerJean.github.io\blogImages\1578384656158.png)
...
![1578384692827](D:\study\HealerJean.github.io\blogImages\1578384692827.png)



### 6.4、jinfo：实时查看Java系统和调整JVM的各项参数

>jinfo 是 JDK 自带的命令，可以用来查看正在运行的 java 应用程序的扩展参数，包括Java System属性和JVM命令行参数；也可以动态的修改正在运行的 JVM 一些参数。



#### 6.4.1、`jinfo  pid`：输出当前 jvm 进程的全部参数和系统属性 
...
#### 6.4.2、`jinfo -flag `： 开启/关闭、设置对应名称的参数、显示Jvm参数值包括默认值 

> 使用 jinfo 可以在不重启虚拟机的情况下，可以动态的修改 jvm 的参数。尤其在线上的环境特别有用。  



##### 6.4.2.1、`./jinfo  -flags  pid`打印所有JVM参数（包括默认值 ）
...
### 6.5、`jmap`：生成虚拟机的堆转储快照（heapdump文件）
...
#### 6.5.1、`jmap -heap pid` ：显示Java堆详细信息
...
#### 6.5.2、 `jmap -histo:live` pid：显示堆对象统计信息 
...
#### 6.5.3、`jmap -dump:live ,format = b file = a.log pid` 
...
![1578294958942](D:\study\HealerJean.github.io\blogImages\1578294958942.png)
...
#### 1.2.3、String.intern()问题 



##### 1.2.3.1、jdk1.6  ：复制的是字符串

```java
String s1 = new String("aaa")+new String("bbb");
s1.intern();
String s2 = "aaabbbb";
System.out.println(s1==s2); //引用和字符串比较
```



##### 1.2.4.2、jdk1.7 ：复制的是引用
