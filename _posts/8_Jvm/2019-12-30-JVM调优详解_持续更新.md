...
1. 当前`classLoader`是否有对应的Chunk且有足够的空间。    
...
因为有`GC`的存在，有些Chunk的数据可能会被回收（Free Chunk），那么这些Chunk属于`committe`的一部分，但不属于`capacity  `       

这些被分配的`Chunk`，第三个`Chunk`，基本很难被100%用完，存在碎片内存的情况，这些Chunk实际被使用的内存之和即used的大小；     
...
##### 8.2.1.2、强制触发`FULL GC `（谨慎使用，考虑腾出一台服务器暂时不给用户使用）
...
### 9.1、`top`：命令确定内存情况，找到CPU过高的Java程序    
...
#### 9.1.2、`P`：根据CPU使用百分比大小进行排序 

![1579177169946](D:\study\HealerJean.github.io\blogImages\1579177169946.png)   



#### 9.2、退出，输入命令`top -p 30030 `：只观察进程为30030，CPU百分比最大的那个Java程序 
...
### 9.2、判断是哪个`java`程序，有可能一台服务器部署了`WEB`多台应用  

#### 9.2.1、`ps -ef |grep PID`或者`ps aux | grep PID`
...
#### 9.2.2、`ps -mp PID -o THREAD,tid,time`ad、打印该进程下线程占用`cpu`情况   

> 定位到线程 TID 30332 占用较高，讲它转化成16进制 767c

```shell
ps -mp 30030 -o THREAD,tid,time | sort -rn 
...
> 建议使用命令 `jstack 30030 > 30030cpu.log` 下载到本地查看，这里显得比较牛一点，就在服务器上直接看了



#### 9.3.1、查看16进制为 `767c`的线程日志  
...
#### 9.3.2、通过定位到代码分析是什么原因导致的，如果看不出来，就看看是不是堆的问题   
