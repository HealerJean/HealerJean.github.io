...
## 1、为什么要使用 `Java` 虚拟机（`JVM`）

> `Java` 虚拟机是 `Java` 语言实现“一次编写，到处运行”（`Write Once`, `Run Anywhere`）这一核心理念的关键技术。其主要价值体现在以下几个方面：

### 1）跨平台能力

- 传统高级语言（如 `C/C++`）需针对不同操作系统和硬件架构分别编译生成特定的机器码。
- `Java` 源代码被编译成一种中间形式——**字节码（`Bytecode`）**，而非直接编译为本地机器指令。
- `JVM` 作为字节码的解释/执行环境，屏蔽了底层操作系统的差异。只要目标平台安装了对应版本的 `JVM`，相同的 `.class` 文件即可运行。

### 2）**安全沙箱机制**

- `JVM` 提供了一个受控的运行环境（即“沙箱”），可限制 `Java` 程序对系统资源（如文件系统、网络、内存）的访问。
- 即使某个 `Java` 应用崩溃或存在恶意行为，通常也不会直接影响宿主操作系统或其他应用程序。



### 3）**自动内存管理**

- `JVM` 内置垃圾回收（`Garbage Collection`, `GC`）机制，自动回收不再使用的对象内存，大幅降低内存泄漏和指针错误的风险。
- 开发者无需手动管理内存分配与释放，提高了开发效率和程序稳定性。



### 4）**抽象的虚拟硬件模型**

> `JVM` 本身是一个**进程**，不是操作系统。它运行在操作系统之上，通过操作系统调度资源。因此，“`JVM` 崩溃不会影响其他应用”这一说法基本成立（除非 `JVM` 本身存在严重 `bug` 或耗尽系统资源）。

- `JVM` 定义了一套虚拟的硬件结构，包括：
  - **栈（`Stack`）**：用于方法调用、局部变量存储；
  - **堆（`Heap`）**：用于对象实例的动态分配；
  - **方法区 / 元空间（`Metaspace`）**：存储类元数据；
  - **程序计数器（`PC Register`）**：记录当前线程执行的字节码指令地址。
- 这种抽象使得 `Java` 程序不依赖于真实 `CPU` 架构（如 x86、ARM）。



## 2、虚拟机一直在做哪些优化

> 随着 `Java` 的广泛应用，`JVM` 在性能、启动速度、内存效率等方面持续演进。主要优化方向包括：
>
...
- 从早期的 `Serial GC` → `Parallel GC` → `CMS`（并发标记清除）→ `G1`（`Garbage-First`）→ `ZGC` / `Shenandoah`（低延迟 `GC`）。
- 目标：减少停顿时间（Stop-The-World）、提高吞吐量、适应大堆内存（TB 级）。
- `ZGC` 和 `Shenandoah` 可实现 **毫秒级停顿**，适用于实时性要求高的场景。



### 2）**字符串与集合优化**

- 字符串常量池、`String deduplication`（`G1` `GC` 中支持）减少重复字符串内存占用。
- 集合类（如 `ArrayList`、`HashMap`）内部结构持续优化。



### 3）**即时编译（`JIT`, `Just-In-Time Compilation`）**

- 初期 `JVM` 采用纯解释执行字节码，速度较慢。
- 现代 `JVM`（如 `HotSpot`）引入 `JIT` 编译器，在运行时将**热点代码**（频繁执行的方法）编译为本地机器码，显著提升执行效率。
- 包括 `Client` `Compiler`（`C1`）和 `Server` `Compiler`（`C2`），以及新一代的 `Graal` 编译器。
...
## 2、`OOP`-`KLASS` 模型：`HotSpot JVM` 如何表示 `Java` 对象

### 1）**`OOP-KLASS` 二元模型**

- **`OOP`（`Ordinary Object Pointer`）**：表示**对象实例本身**，存储在 **`Java` 堆（`Heap`）** 中。

- **`KLASS`**：表示**对象的类型元数据（类信息）**，存储在 **元空间（`Metaspace`）**

### 2）核心组件

- 元数据—— `instanceKlass` 对象会存在元空间（方法区）
  - ：`instanceKlass` 是 `C++` 对象，**不能被 `Java` 代码直接访问**。

- 对象实例——` instanceOopDesc` 会存在 `Java` 堆。`Java` 虚拟机栈中会存有这个对象实例的引用



### 3）创建对象过程

#### a、步骤 1：类加载 → 创建 `instanceKlass`

- 当 `JVM` 首次遇到某个类（如通过 `new`、`invokestatic`、反射等），若该类未被加载：
  - 触发**类加载机制**（加载 → 验证 → 初始化）。
  - 在 **元空间** 中创建一个 `instanceKlass` 对象（`C++` 层）。
  - 该 `instanceKlass` 包含：
    - 类名、访问标志
    - 字段表、方法表
    - 常量池（运行时常量池）
    - 超类、接口信息
    - **指向其 `Java` 镜像（`java.lang.Class` 对象）的引用**（字段 `_java_mirror`）



#### b、步骤 2：`new` 对象 → 创建 `instanceOopDesc`

- 执行 `new MyClass()` 时：
  - `JVM` 在 **`Java` 堆** 中分配内存，创建一个 `instanceOopDesc` 实例。
  - 该对象的**对象头（`Object Header`）** 中包含一个 **`_klass` 指针**，指向对应的 `instanceKlass`。
  - 方法区（元空间）中的 `instanceKlass` 提供类型信息（用于方法分派、字段偏移计算等）。
- 同时，在当前线程的 **`Java` 虚拟机栈** 中，局部变量表会保存一个对该 `instanceOopDesc` 的引用（即我们常说的“对象引用”）。



#### c、`java.lang.Class` 对象的创建（`Java` 镜像）

- `HotSpot` 不会直接暴露 `instanceKlass` 给 `Java` 层。

- 而是在类加载完成后，**在 `Java` 堆中创建一个 `java.lang.Class` 的实例**（本质也是一个 `instanceOopDesc`）。

  - 这个 `Class` 对象被称为 `instanceKlass` 的 **“`Java` 镜像”（`Java Mirror`）**。
  - `instanceKlass` 通过 `_java_mirror` 字段持有对该 `Class` 对象的引用。

- 反过来，`Java` 层通过 `MyClass.class` 或 `obj.getClass()` 获取的就是这个堆中的 `Class` 实例。

  ```java
  MyClass obj = new MyClass();
  Class<?> clazz = obj.getClass(); // 返回的就是 instanceKlass 的 Java 镜像
  ```
...
- 所有 `MyClass` 的实例都共享同一个 `instanceKlass`。
- 所有对 `MyClass.class` 的访问都指向同一个 `java.lang.Class` 实例。

```
[Java Stack] 
   │
   └── 引用 ──► [instanceOopDesc] (堆中对象实例)
                     │
                     ▼ (_klass 指针)
               [instanceKlass] (元空间，类元数据)
                     │
                     ▼ (_java_mirror)
               [instanceOopDesc of java.lang.Class] (堆中 Class 对象)
```
...
## 3、`HotSpot` 中 `Java` 对象的内存布局

> 在 `HotSpot` 虚拟机中，一个普通的 `Java` 对象（非数组）在堆内存中的布局由三部分组成：

![1578561760566](/Users/zhangyujin1/Desktop/HealerJean/HCode/HealerJean.github.io/blogImages/1578561760566-5332175.png)

### 1）对象头（`Object Header`）

> 对象头是每个 `Java` 对象都必须包含的元信息区域，用于支持 `JVM` 的运行时机制（如 `GC`、同步、类型识别等）。它通常包含以下两个（或三个）字段：

#### a、`Mark` `Word`（标记字）

- **作用**：存储对象的运行时元数据。
- **内容随对象状态动态变化**（通过“锁状态”标志位切换含义），典型内容包括：
  - **`HashCode`**（若已计算）
  - **`GC` 分代年龄**（用于分代收集器判断是否晋升老年代）
  - **锁状态标志**：
    - 无锁
    - 偏向锁（Biased Locking）→ 包含线程 ID、Epoch
    - 轻量级锁（Lightweight Locking）→ 指向栈中 Lock Record 的指针
    - 重量级锁（Inflated）→ 指向 Monitor 对象的指针
  - **`GC` 标记位**（如是否被标记为存活）
- **大小**：
  - `32` 位 `JVM`：4 字节
  - `64` 位 `JVM`：8 字节（默认）
  - 若启用 **压缩指针（Compressed Oops）**，仍为 8 字节（但部分位用于其他用途）



#### b、`Klass` `Pointer`（类元数据指针）

- **作用**：
  - 每个 `Java` 对象的对象头中包含一个指针，指向其类型元数据（即 C++ 层的 `instanceKlass`）。
  - `JVM` 通过它知道“这个对象属于哪个类”，用于方法调用、类型检查、GC 等。
- **位置**：紧随 `Mark Word` 之后。
- **大小**：
  - 默认 `64` 位指针：**4 字节**
...
#### c、`Array` `Length`（仅数组对象有）

- **作用**：记录数组的元素个数。
- **大小**：4 字节（int 类型）
  - 数组创建语法：`new T[length]`，其中 `length` 必须是 `int` 表达式

- **位置**：在 Klass Pointer 之后（即对象头的第三部分）
- **普通对象没有此字段**
  - 普通对象头 = Mark Word + Klass Pointer
  - 数组对象头 = Mark Word + Klass Pointer + Array Length



### 2）实例数据（`Instance Data`）

- **内容**：对象中所有**非静态字段**的实际值，包括：
  - 本类定义的字段
  - 从父类继承的字段（按继承链向上合并）
- **字段排列规则**：
  - 默认按 **字段宽度从大到小排序，以减少内存对齐开销。**
    - `long/double` → `int/float` → `short/char` → `byte/boolean` → `reference`
  - 可通过 `-XX:+CompactFields`（默认开启）将子类窄字段“插入”父类宽字段间隙，进一步压缩内存。
- **引用类型字段**：存储的是指向其他堆对象的 `OOP` 引用（可能被压缩为 `32` 位，若启用 Compressed Oops）。



### 3）对齐填充（`Padding`）

- **目的**：保证对象总大小是 **`8` 字节的倍数**（`HotSpot` 要求对象起始地址按 `8` 字节对齐，以提升 `CPU` 访问效率）。
- **实现方式**：在实例数据后填充若干字节（0~7 字节）。
- **无业务含义**，纯为性能优化。
- **并非所有对象都需要填充**：若实例数据 + 对象头 已是 `8` 的倍数，则无填充。
...
> 在 `64` 位系统中，一个“字”通常是 **8 字节**。如果数据地址是 8 的倍数，`CPU` 可以**一次读取完成**；
>
> >  如果地址不对齐，可能需要**两次内存访问 + 拼接**，速度变慢。因此，`HotSpot` 要求：**每个 `Java` 对象的起始地址必须是 8 字节对齐的**。

- **字段的自然对齐要求**
  - `long` / `double`：最好 8 字节对齐（偏移是 8 的倍数）
  - `int` / `float`：最好 4 字节对齐（偏移是 4 的倍数）
  - `short` / `char`：2 字节对齐
  - `byte` / `boolean`：任意对齐
- **整体对象大小必须是 8 字节的倍数**
...
> 虽然 8 + 4 = 12，但 **HotSpot 要求对象整体按 8 字节对齐**，所以后续的实例数据会从偏移 16 开始（中间可能有 4 字节空隙），但**对象头本身仍是 12 字节**。

| 组成部分        | 大小（字节） | 说明                                                |
| --------------- | ------------ | --------------------------------------------------- |
| `Mark Word`     | 8            | 存储 `hash`、锁、GC 年龄等                          |
| `Klass Pointer` | 4            | 压缩类指针（`UseCompressedClassPointers` 默认开启） |
| **总计**        | **`12`**     | 对象头最小尺寸                                      |

| 对象类型     | 对象头最小字节数（64 位默认 JVM） |
| ------------ | --------------------------------- |
| **普通对象** | **12 字节**（8 + 4）              |
| **数组对象** | **16 字节**（8 + 4 + 4）          |
...
> 数组对象头天然对齐到 8 字节倍数（16 是 8 的倍数），无需额外填充。
...
| 组成部分     | 内容                              | 大小（字节）  |
| ------------ | --------------------------------- | ------------- |
| **对象头**   | `Mark Word` + `Klass Pointer`     | **12**        |
| **实例数据** |                                   |               |
|              | `modCount`（来自 `AbstractList`） | 4             |
|              | `size`（`ArrayList `自有）        | 4             |
|              | `elementData`（引用）             | 4（压缩指针） |
| **对齐填充** | 使总大小为 `8` 的倍数             | **0**         |
| **总计**     |                                   | **24 字节**   |
...
> **那么变量在准备阶段过后的初始值为0，而不是123，因为这个时候还没有执行 `java` 方法，123的动作在初始化阶段才会执行。**
...
#### a、自己编写的是由 `AppClassLoader` 加载的。  

>  **解释：也就是说明 `Jvm02Test.class` 文件是由 `AppClassLoader` 加载的。**
...
#### b、系统自带的 `int` `String`  通过`Bootstrap ClassLoader`加载呢  
...
>  **解释 ：这个说明，`AppClassLoader` 的父加载器是 `ExtClassLoader` 。那么 `ExtClassLoader` 的父加载器又是谁呢？**
...
#### d、`ExtClassLoader` 的父加载器  

> **解释：又是一个空指针异常，这表明 `ExtClassLoader` 也没有父加载器。那么，为什么标题又是每一个加载器都有一个父加载器呢？这不矛盾吗？为了解释这一点，我们还需要看下面的一个基础前提。**  
...
#### b、 **`JVM` 在搜索类的时候，又是如何判定两个class是相同的呢？**     
...
#### c、自定义 `ClassLoader` 示例之 `DiskClassLoader`。  
...
### 3）`ClosserLoader` 卸载 `Class`  

>  `JVM` 中的 `Class` 只有满足三个条件，才能被 `GC` 回收，也就是该 `Class` 被卸载（unload）：看上面 
...
#### a、服务器启动共用一个 `AppClassLoader `

> 返回结果只有一个 `Classloader`，所以一般类很难被卸载，因此才到了方法区 
...
> 经常有人把 `java` 内存区分为堆内存和栈内存，这种分发是错误的，只能说明大多数程序员最密切的就是这两块，<font color="red">在多线程环境下，每个线程拥有一个栈和一个程序计数器。栈和程序计数器用来保存线程的执行历史和线程的执行状态，是线程私有的资源。其他的资源（比如堆、方法区）是由同一个进程内的多个线程共享。 </font>
...
> 本地方法栈和虚拟机栈区别是，虚拟机栈为执行的 `java` 方法服务，而本地方法栈为 `Native` 方法服务。它也会抛出`StackOverflowError`和`OutOfMemoryError`异常
...
##### 3）测试  ：同时设置 PermSize 和 MaxPermSize的大小。     
...
> 一般**由程序员分配释放**，   若程序员不释放，程序结束时可能由 `OS` 回 收，**堆内存用来存放由new创建的对象和数组。在堆中分配的内存，（包括类的成员变量上）**    
...
@：当声明是基本类型的变量的时，其变量名及值（变量名及值是两个概念）是放在 `JAVA` 虚拟机栈中   

@：当声明的是引用变量时，所声明的变量（该变量实际上是在方法中存储的是内存地址值）是放在 `JAVA` 虚拟机的栈中，该变量所指向的对象是放在堆类存中的。  



#### d、 在类中声明的变量是成员变量，也叫全局变量
