...
```
答：在系统A登录成功后，用户和认证中心之间建立起了全局会话，这个全局会话就是TGT(Ticket Granting Ticket)，TGT位于CAS服务器端，TGT并没有放在客户端Session中，也就是说，CAS全局会话的实现并没有直接使用Session机制，而是利用了Cookie自己实现的，这个Cookie叫做TGC(Ticket Granting Cookie)，它存放了TGT的id,保存在用户浏览器上。  
...
```
1、先说ST：ST的过期包括使用次数和时间，默认使用一次就过期，或者即使没有使用，一段时间后也要过期，st默认过期时间是10秒，

2、TGT是可以多次使用的，TGT的默认过期时间是7200秒=2小时

客户端长时间不操作，就会重新登录?
那是因为我们的本地session过期了，正常情况下session的过期时间是30分钟，30分钟不操作就过期了，如果我们设置的session过期时间特别长会怎么办。那就一直可以操作啊，傻瓜，即使服务器过期了，客户端都不会过期。以为不需要再向服务端发起登录申请了
