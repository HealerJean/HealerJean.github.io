...
- **系统启动时：**记录系统或者服务的启动参数，核心模块或组件初始化的关键配置，以及启动完成的服务状态描述信息。
- **系统异常时：**系统抛出的异常通常是告知研发要加以关注的信息，应根据实际情况并结合业务状况记录 WARN 或者 ERROR 级别日志。
- **业务异常时：**所有业务流程分支都应考虑记录日志，常见场景包括请求参数不正确，第三方服务调用返回码不在预期内等。
- **关键动作时：**系统中核心角色触发的业务动作是需要多加关注的，无论是故障定位还是审计都有关键作用。
- **远程调用时：**永远不要信任第三方服务的稳定性，对三方服务的调用建议打印关键请求和响应参数，方便和各个终端定位问题。
...
| 级别        | 说明                                                         |
| ----------- | ------------------------------------------------------------ |
| **`TRACE`** | 记录各接口的出入参，该级别日志主要用于在开发、测试阶段输出，生产环境原则上不应打印 `TRACE` 级别日志。 |
...
```
//export/Logs/${应用英文名}
│ ── common-default.log 由业务代码主动输出&框架自动输出
│ ── common-error.log 只保存ERROR级别日志及上下文堆栈，由业务代码主动输出&框架自动输出，用于错误码大盘及故障排查
