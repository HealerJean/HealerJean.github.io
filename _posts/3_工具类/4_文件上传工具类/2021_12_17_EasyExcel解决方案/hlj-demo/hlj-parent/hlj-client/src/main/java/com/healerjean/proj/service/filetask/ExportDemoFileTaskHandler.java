...
public class ExportDemoFileTaskHandler extends AbstractExportFileTaskHandler {
    ...
    public FileTaskResultBO executeExportTask(FileTaskBO fileTask) throws Exception {
        ...
        PageQueryBO<UserDemoQueryBO> pageQuery = new PageQueryBO<>();
        pageQuery.setPageSize(1L);
        pageQuery.setSearchCountFlag(false);
        pageQuery.setData(queryBO);
        ExcelWriteUtils excelWriteUtils = ExcelWriteUtils.instance(ExcelEnum.ExcelObjEnum.EXPORT_EXCEL_USER_DEMO);
        excelWriteUtils.pageWrite(pageNo -> {
            pageQuery.setCurrPage(pageNo);
            List<UserDemoBO> list = userDemoService.queryUserDemoPage(pageQuery).getList();
            return  UserDemoConverter.INSTANCE.covertUserDemoPoToExcelList(list);
        });
