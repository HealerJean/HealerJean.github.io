...
public interface UserDemoConverter {
     ...
     * @return List<UserDemoBO>
     */
    default List<UserDemoExportExcel> covertUserDemoPoToExcelList(List<UserDemoBO> userDemos) {
        if (CollectionUtils.isEmpty(userDemos)) {
            return Collections.emptyList();
        }
    ...
    default UserDemoExportExcel covertUserDemoPoToExcel(UserDemoBO userDemo) {
        if (Objects.isNull(userDemo)) {
            return null;
        }
        UserDemoExportExcel result = new UserDemoExportExcel();
        result.setId(userDemo.getId());
        result.setName(userDemo.getName());
        result.setAge(userDemo.getAge());
        result.setPhone(userDemo.getPhone());
