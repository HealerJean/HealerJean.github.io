...
public class AuthRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        //1、获取登录后需要的的基本信息（将来无需查询数据库直接获取）
        LoginUserDTO loginUserDTO = identityService.getUserInfo(token.getUserId());
        log.info("获取到LoginUser信息：{}", loginUserDTO);
        //返回的用户身份信息
        SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo();

        //2、shiro中放入用户权限
        List<MenuDTO> permissions = loginUserDTO.getPermissions();
        List<MenuDTO> menus = loginUserDTO.getMenus();
        ...
        principalCollection.add(collect, loginUserDTO.getRealName());
        ...
        loginUserDTO.setMenus(null);
        loginUserDTO.setPermissions(null);
        session.setAttribute(AuthConstants.AUTH_USER, loginUserDTO);
